...
export default function MyTimesheet() {
  ...
  const load = () => {
    setLoading(true);
    return getMyTimesheets({ month, year }, token).then(setRows).finally(() => setLoading(false));
  };
  ...
  const handleSave = async (e) => {
    ...
    setError('');
    try {
      if (editingId === 'new') await addMyTimesheet(form, token);
      else await updateMyTimesheet(editingId, form, token);
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    setSaving(true);
    try {
      await deleteMyTimesheet(id, token);
      await load();
    } finally {
      setSaving(false);
    }
    ...
    <div className="page">
      ...
      <div className="timesheet-tab">
        ...
        <div className="timesheet-summary">
          ...
          <div className="summary-card">
            <div className="summary-value">{daysWorked > 0 ? (totalHours / daysWorked).toFixed(1) : '—'} h</div>
        ...
        {loading ? (
          ...
          <p className="empty-state">Aucun pointage saisi pour cette période.</p>
        ) : (
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 420 }}>
            ...
            <form onSubmit={handleSave}>
              <div className="field">
                <label>Date *</label>
                <input type="date" value={form.date} onChange={(e) => setForm(f => ({ ...f, date: e.target.value }))} disabled={editingId !== 'new'} required />
              </div>
              ...
              <div className="field">
                ...
                <input type="number" min="0" value={form.breakMinutes} onChange={(e) => setForm(f => ({ ...f, breakMinutes: e.target.value }))} />
              </div>
              <div className="field">
                <label>Note <span className="hint">(optionnel)</span></label>
                <input type="text" value={form.note} onChange={(e) => setForm(f => ({ ...f, note: e.target.value }))} placeholder="Ex : télétravail…" />
              </div>
              {error && <p className="error-msg">{error}</p>}
              <div className="form-actions">
                <button type="button" className="btn-ghost" onClick={() => setEditingId(null)} disabled={saving}>Annuler</button>
